...
import { Code2, Search, LogOut } from 'lucide-react';
                        ...
                        {status === 'loading' ? (
                            ...
                            <>
                                {session.user.role === 'admin' && (
                                    <Link href="/admin/dashboard" className="text-sm font-semibold text-gray-600 hover:text-indigo-600 transition-colors hidden sm:block">
                                        Dashboard
                                    </Link>
                                )}
                                <div className="flex items-center gap-4 pl-6 border-l border-gray-200">
                                    <span className="text-sm font-medium text-gray-900 hidden sm:block">{session.user.email?.split('@')[0]}</span>
                                    <button
