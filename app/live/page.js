...
import {
    ...
    Code, Copy, CheckCircle2, Link as LinkIcon, Activity, FileCode2
} from 'lucide-react';
...
export default function LivePage() {
    ...
    const [activeTab, setActiveTab] = useState('chat'); // 'chat' or 'playground'
    ...
    useEffect(() => {
        fetchStream();
        fetchFollowData();
        pollRef.current = setInterval(fetchStream, 60000);
            ...
            <div className="h-screen w-full bg-[#09090b] flex flex-col overflow-hidden">
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center px-6 relative overflow-y-auto">
                    ...
                    <div className="relative z-10 w-full max-w-4xl mx-auto flex flex-col items-center">
                        ...
                        <button onClick={toggleFollow} className={`flex items-center gap-3 px-8 py-4 rounded-2xl font-bold text-lg transition-all shadow-xl ${isFollowing ? 'bg-white/10 text-white border border-white/10' : 'bg-indigo-600 text-white hover:bg-indigo-500 hover:shadow-indigo-600/30'}`}>
                            {isFollowLoading ? <div className="w-6 h-6 border-2 border-current border-t-transparent rounded-full animate-spin" /> : <><Star size={22} className={isFollowing ? "fill-white" : ""} />{isFollowing ? 'Following Channel' : 'Follow to get notified'}</>}
                        </button>
                    </div>
                </div>
            </div>
    ...
    return (
        <div className="h-screen w-full bg-[#09090b] flex flex-col overflow-hidden selection:bg-indigo-500/30 font-sans">
            ...
            <div className="flex-1 flex flex-col lg:flex-row min-h-0 relative">
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-6 pb-6 border-b border-white/10">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 p-[2px]">
                                        <div className="w-full h-full bg-gray-900 rounded-full flex items-center justify-center text-white font-bold text-lg">LM</div>
                                    </div>
                                    <div>
                                        <h3 className="text-lg font-bold text-white flex items-center gap-2">LearnMade <svg className="w-4 h-4 text-blue-500" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10 10-4.5 10-10S17.5 2 12 2zm-1.9 14.7L6 12.6l1.5-1.5 2.6 2.6 6.4-6.4 1.5 1.5-7.9 7.9z"/></svg></h3>
                                        <p className="text-sm text-gray-400 font-medium">{followersCount.toLocaleString()} followers</p>
                                    </div>
                                </div>
                                <button onClick={toggleFollow} disabled={isFollowLoading} className={`flex items-center justify-center gap-2 px-6 py-2.5 rounded-full font-bold text-sm transition-all ${isFollowing ? 'bg-white/10 text-white hover:bg-white/15' : 'bg-white text-black hover:bg-gray-200'}`}>
                                    {isFollowLoading ? <div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" /> : <>{isFollowing && <Star size={16} className="fill-white" />} {isFollowing ? 'Following' : 'Follow'}</>}
                                </button>
                            </div>
                            ...
                            {chapters.length > 0 && (
                                <div className="mt-8">
                                    <h3 className="text-sm font-bold text-gray-300 uppercase tracking-wider mb-6 flex items-center gap-2">
                                        <Activity size={16} className="text-indigo-400" /> Live Chapters
                                    </h3>
                                    <div className="space-y-4">
                                        {chapters.map((c, idx) => (
                                            <div key={c._id} className="relative pl-6 pb-4 border-l-2 border-indigo-600/30 last:border-0 last:pb-0">
                                                <div className="absolute -left-[9px] top-0 w-4 h-4 bg-[#09090b] border-2 border-indigo-500 rounded-full" />
                        ...
                        <div className="xl:w-80 shrink-0">
                            <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-5 relative overflow-hidden">
                                <div className="flex items-center justify-between mb-4 relative z-10">
                                    ...
                                    {totalReactions > 0 && <span className="text-xs text-gray-500 font-bold bg-white/5 px-2 py-1 rounded-md">{totalReactions.toLocaleString()}</span>}
                                </div>
                                ...
                                <div className="relative z-10">
                                    ...
                                    <div className="grid grid-cols-4 gap-2">
                                        {REACTION_LIST.map(({ emoji, key }) => (
                                            <button key={key} onClick={() => handleReaction(emoji, key)} className="group relative flex flex-col items-center justify-center gap-1.5 p-3 bg-white/5 hover:bg-white/10 active:scale-95 rounded-xl transition-all border border-transparent hover:border-white/10">
                ...
                {/* ── RIGHT: Chat / Playground Tabs ── */}
                <div className="w-full lg:w-[420px] xl:w-[480px] shrink-0 border-t lg:border-t-0 lg:border-l border-white/10 bg-[#0f0f12] flex flex-col h-[50vh] lg:h-full relative z-20">
                    
                    {/* Tab Navigation */}
                    <div className="flex items-center px-4 pt-3 border-b border-white/5 bg-[#09090b]">
                        <button 
                            onClick={() => setActiveTab('chat')}
                            className={`flex-1 flex items-center justify-center gap-2 pb-3 border-b-2 transition-all font-bold text-sm ${activeTab === 'chat' ? 'border-indigo-500 text-white' : 'border-transparent text-gray-500 hover:text-gray-300'}`}
                        >
                            <MessageSquare size={16} /> Q&A Chat
                        </button>
                        <button 
                            onClick={() => setActiveTab('playground')}
                            className={`flex-1 flex items-center justify-center gap-2 pb-3 border-b-2 transition-all font-bold text-sm ${activeTab === 'playground' ? 'border-indigo-500 text-white' : 'border-transparent text-gray-500 hover:text-gray-300'}`}
                        >
                            <FileCode2 size={16} /> Playground
                        </button>
                    </div>
                    ...
                    {activeTab === 'chat' && (
                        <>
                            <div className="px-5 py-2 border-b border-white/5 bg-[#0a0a0c] flex items-center justify-between">
                                <span className="text-xs text-gray-500 font-medium">Chat is heavily moderated.</span>
                                                    ...
                                                    {msg.isCodeSnippet ? (
                                                        <div className="bg-[#1e1e1e] rounded-lg border border-white/10 overflow-hidden relative group/code">
                                                            <div className="flex justify-between items-center px-3 py-1.5 bg-black/40 border-b border-white/5">
                                                                ...
                                                                <button onClick={() => copyToClipboard(msg._id, msg.text)} className="text-gray-400 hover:text-white transition-colors">
                                                        ...
                                                        <p className="text-[13px] text-gray-200 leading-relaxed break-words">{msg.text}</p>
                                                    )}
                                                </div>
                                                <button onClick={() => upvoteMessage(msg._id)} disabled={alreadyUpvoted} className={`flex flex-col items-center justify-center gap-0.5 w-8 h-8 rounded-lg transition-all shrink-0 mt-1 ${alreadyUpvoted ? 'text-indigo-400 cursor-default' : 'text-gray-600 hover:text-white hover:bg-white/5'}`}>
                            ...
                            <div className="shrink-0 p-4 border-t border-white/5 bg-[#09090b]">
                                <form onSubmit={submitQuestion} className="flex flex-col gap-3">
                                    <div className="flex items-center justify-between">
                                        {!session?.user?.name ? (
                                            <input value={questionName} onChange={e => setQuestionName(e.target.value)} placeholder="Display name" maxLength={50} className="w-1/2 px-3 py-1.5 bg-white/5 border border-transparent rounded-md text-xs text-white placeholder:text-gray-600 focus:outline-none focus:border-white/20 transition-all" />
                                        ) : <div />}
                                        <button type="button" onClick={() => setIsCodeMode(!isCodeMode)} className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold transition-all ml-auto ${isCodeMode ? 'bg-indigo-600 text-white' : 'bg-white/5 text-gray-400 hover:text-white hover:bg-white/10'}`}>
                                    ...
                                    <div className="relative flex bg-white/5 border border-white/10 focus-within:border-indigo-500 rounded-lg overflow-hidden transition-colors">
                                        {isCodeMode ? (
                                            <textarea value={question} onChange={e => setQuestion(e.target.value)} placeholder="Paste your code snippet here..." className="w-full h-24 pl-3 pr-10 py-3 bg-transparent text-xs text-gray-300 font-mono placeholder:text-gray-600 focus:outline-none resize-none custom-scrollbar" />
                                        ) : (
                                            <input value={question} onChange={e => setQuestion(e.target.value)} placeholder="Chat publicly..." maxLength={500} className="w-full pl-3 pr-10 py-3 bg-transparent text-sm text-white placeholder:text-gray-500 focus:outline-none" />
                                        )}
                                        <button type="submit" disabled={!question.trim() || sending} className={`absolute right-1 bottom-1 p-2 rounded-md transition-all ${!question.trim() || sending ? 'opacity-30' : 'text-indigo-400 hover:bg-indigo-500/10 hover:text-indigo-300'}`}>
                    ...
                    {activeTab === 'playground' && (
                        <div className="flex-1 overflow-hidden bg-[#151515] flex flex-col">
                            <div className="px-4 py-3 border-b border-white/5 bg-[#0a0a0c]">
                                <h3 className="text-xs font-bold text-gray-300 mb-1">Live Sandbox</h3>
                                <p className="text-[10px] text-gray-500">Write and test code alongside the stream.</p>
                            </div>
